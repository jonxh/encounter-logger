 ...
 * Falls back to console (stdout) if file creation fails or file logging is
 * disabled.
 *
...
public class EncounterLogWriter {
    ...
    private static final DateTimeFormatter TIMESTAMP_FMT = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HHmmss")
    ...
    public void open(String label, int worldId, boolean toFile) {
        ...
        try {
            ...
            log.warn("[EncounterLogger] Could not open file {}: {} — falling back to console.", currentPath,
                    e.getMessage());
            writer = null;
            fileMode = false;
        }
    ...
    /**
     * @return the absolute path of the current log file, or null if not writing to
     *         a file.
     */
