...
public class EncounterRecorder {
    ...
    private Map<String, Object> prevPlayerSnap = null;
    ...
    // Blocked object IDs — scene objects whose IDs appear in this set are never
    // logged.
    // Populated from config at recording start.
    private final Set<Integer> blockedObjectIds = new HashSet<>();

    // Player attack tracking — detect when animation changes to emit player_attack
    // events.
    ...
    private void checkPlayerAttack() {
        ...
        // Only emit when animation changes to something non-idle, we have a target,
        // and it is not a known non-attack animation like eating (829) or drinking.
        if (anim != -1 && anim != prevPlayerAnim && target != null && anim != 829) {
    ...
    private void emitTickSnapshot() {
        ...
        if (lp != null) {
            ...
            event.put("playerWorld",
                    EncounterJson.worldPoint(canonical.getX(), canonical.getY(), canonical.getPlane()));
            event.put("playerScene", EncounterJson.localPoint(lp.getSceneX(), lp.getSceneY(), canonical.getPlane()));
        }

        if (config.logPlayerState()) {
            Map<String, Object> pSnap = buildPlayerSnapshot(player);
            checkPlayerStateChanges(pSnap);
            event.put("player", pSnap);
        }
        ...
        // TODO: Future: optional collision/blocked tile logging around player or object spawns.
        
        writer.writeLine(event);
    }

    private void checkPlayerStateChanges(Map<String, Object> pSnap) {
        if (prevPlayerSnap != null) {
            if (!Objects.equals(prevPlayerSnap.get("hp"), pSnap.get("hp"))) {
                emitPlayerStateChange("player_hp_changed", "hp", prevPlayerSnap.get("hp"), pSnap.get("hp"));
            }
            if (!Objects.equals(prevPlayerSnap.get("prayer"), pSnap.get("prayer"))) {
                emitPlayerStateChange("player_prayer_changed", "prayer", prevPlayerSnap.get("prayer"), pSnap.get("prayer"));
            }
            if (!Objects.equals(prevPlayerSnap.get("specEnergy"), pSnap.get("specEnergy"))) {
                emitPlayerStateChange("player_spec_changed", "specEnergy", prevPlayerSnap.get("specEnergy"), pSnap.get("specEnergy"));
            }
            if (!Objects.equals(prevPlayerSnap.get("attackStyle"), pSnap.get("attackStyle"))) {
                emitPlayerStateChange("player_attack_style_changed", "attackStyle", prevPlayerSnap.get("attackStyle"), pSnap.get("attackStyle"));
            }
            if (!Objects.equals(prevPlayerSnap.get("activePrayers"), pSnap.get("activePrayers"))) {
                emitPlayerStateChange("player_active_prayers_changed", "activePrayers", prevPlayerSnap.get("activePrayers"), pSnap.get("activePrayers"));
            }
        }
        prevPlayerSnap = pSnap;
    }

    private void emitPlayerStateChange(String type, String field, Object prev, Object current) {
        Map<String, Object> event = EncounterJson.base(type, sessionId, localTick);
        event.put("field", field);
        event.put("previous", prev);
    ...
    private Map<String, Object> buildPlayerSnapshot(Player player) {
        ...
        m.put("runMode", client.getVarpValue(173)); // 0 = walk, 1 = run
        ...
        // Prayer drain detection — emit separate event if prayer drained > 2 points
        // this tick
    ...
    private List<Map<String, Object>> buildNearbyNpcList(WorldPoint playerWp) {
        ...
        for (NPC npc : client.getNpcs()) {
            ...
            if (npcWp == null)
                continue;
            if (!isWithinRadius(playerWp, npcWp, radius))
                continue;
    ...
    private Map<String, Object> buildNpcSnapshot(NPC npc) {
        ...
        Map<String, Object> snap = EncounterJson.npcSnapshot(
                npc.getIndex(),
                npc.getId(),
                npc.getName(),
        ...
        if (comp != null) {
            ...
            if (transformed != null) {
                transformedId = transformed.getId();
            }
            snap.put("composition", EncounterJson.npcCompositionSnapshot(
                    comp.getCombatLevel(), comp.getSize(), comp.getActions(), transformedId));
        }
    ...
    private void emitNpcStateChanges(List<Map<String, Object>> currentNpcs) {
        ...
        for (Map<String, Object> snap : currentNpcs) {
            ...
            if (prev.animation != animation) {
                changes.put("animation", new int[] { prev.animation, animation });
                prev.animation = animation;
            }
            if (prev.poseAnimation != poseAnimation) {
                changes.put("poseAnimation", new int[] { prev.poseAnimation, poseAnimation });
                prev.poseAnimation = poseAnimation;
            }
            if (prev.graphic != graphic) {
                changes.put("graphic", new int[] { prev.graphic, graphic });
                prev.graphic = graphic;
            }
            if (prev.healthRatio != healthRatio) {
                changes.put("healthRatio", new int[] { prev.healthRatio, healthRatio });
                
                if (config.logNpcHealthChanges() && prev.healthRatio != -1 && healthRatio != -1) {
                    Map<String, Object> hpEvent = EncounterJson.base("npc_health_changed", sessionId, localTick);
                    hpEvent.put("npcIndex", index);
                    hpEvent.put("npcId", snap.get("id"));
                    hpEvent.put("npcName", snap.get("name"));
                    hpEvent.put("worldLocation", snap.get("worldLocation"));
                    hpEvent.put("previousHealthRatio", prev.healthRatio);
                    hpEvent.put("currentHealthRatio", healthRatio);
                    hpEvent.put("healthScale", healthScale);
                    hpEvent.put("deltaRatio", healthRatio - prev.healthRatio);
                    hpEvent.put("direction", (healthRatio > prev.healthRatio) ? "healed" : "damaged");
                    hpEvent.put("interactingName", interacting);
                    writer.writeLine(hpEvent);
                }
                
                prev.healthRatio = healthRatio;
            }
            if (prev.healthScale != healthScale) {
                changes.put("healthScale", new int[] { prev.healthScale, healthScale });
                prev.healthScale = healthScale;
            }
            if (prev.orientation != orientation) {
                changes.put("orientation", new int[] { prev.orientation, orientation });
                prev.orientation = orientation;
            }
            if (!Objects.equals(prev.overheadText, overhead)) {
                changes.put("overheadText", new String[] { prev.overheadText, overhead });
                prev.overheadText = overhead;
            }
            if (!Objects.equals(prev.interactingName, interacting)) {
                changes.put("interactingName", new String[] { prev.interactingName, interacting });
                prev.interactingName = interacting;
            }
            if (!Objects.equals(prev.overheadIcon, overheadIcon)) {
                changes.put("overheadIcon", new String[] { prev.overheadIcon, overheadIcon });
                prev.overheadIcon = overheadIcon;
            }
        ...
        // Remove NPCs that left radius (not a game despawn, but they're no longer
        // tracked)
    ...
    public void onNpcSpawned(NpcSpawned e) {
        if (!recording || !config.logNpcEvents())
            return;
        NPC npc = e.getNpc();
    ...
    public void onHitsplatApplied(HitsplatApplied e) {
        if (!recording || !config.logHitsplats())
            return;
    ...
    public void onAreaSoundEffectPlayed(AreaSoundEffectPlayed e) {
        ...
        WorldPoint wp = (src != null)
                ...
                : null;
        if (wp != null)
            event.put("sourceWorldLocation", EncounterJson.worldPoint(wp.getX(), wp.getY(), wp.getPlane()));
        writer.writeLine(event);
    }

    // ── Menu Options / Red Clicks ─────────────────────────────────────────────

    public void onMenuOptionClicked(MenuOptionClicked e) {
        if (!recording || !config.logMenuActions())
            return;

        Map<String, Object> event = EncounterJson.base("menu_click", sessionId, localTick);
        event.put("option", e.getMenuOption());
        event.put("target", e.getMenuTarget());
        event.put("id", e.getId());
        event.put("action", e.getMenuAction().name());
        event.put("param0", e.getParam0());
        event.put("param1", e.getParam1());
        event.put("itemId", e.getItemId());
        if (e.getWidget() != null) {
            event.put("widgetId", e.getWidget().getId());
        }
        writer.writeLine(event);
    }
    ...
    public void onChatMessage(ChatMessage e) {
        if (!recording || !config.logChatMessages())
            return;
    ...
    private void emitProjectiles() {
        ...
        for (Projectile p : projectiles) {
            ...
            if (player != null && player.getLocalLocation() != null) {
                WorldPoint canonical = instanceAwareWorldPoint(player.getLocalLocation());
                event.put("playerWorldLocation", EncounterJson.worldPoint(canonical.getX(), canonical.getY(), canonical.getPlane()));
            }
            
            // Try to guess source coords from projectile start
            int startX = (int) p.getX();
            int startY = (int) p.getY();
            int srcWorldX = -1, srcWorldY = -1;
            WorldPoint srcWp = null;
            if (startX > 0 && startY > 0) {
                srcWp = WorldPoint.fromLocal(client, startX, startY, client.getPlane());
                srcWorldX = srcWp.getX();
                srcWorldY = srcWp.getY();
            }
            ...
            // Find nearest NPCs
            if (srcWp != null) {
                NPC nearestSrc = findNearestNpc(srcWp);
                if (nearestSrc != null) {
                    event.put("nearestNpcToSourceId", nearestSrc.getId());
                    event.put("nearestNpcToSourceName", nearestSrc.getName());
                }
            }
            if (targetWp != null) {
                NPC nearestTgt = findNearestNpc(targetWp);
                if (nearestTgt != null) {
                    event.put("nearestNpcToTargetId", nearestTgt.getId());
                    event.put("nearestNpcToTargetName", nearestTgt.getName());
                }
            }
    ...
    private NPC findNearestNpc(WorldPoint wp) {
        NPC nearest = null;
        int minDist = Integer.MAX_VALUE;
        for (NPC npc : client.getNpcs()) {
            if (npc == null || npc.getWorldLocation() == null) continue;
            int dist = npc.getWorldLocation().distanceTo(wp);
            if (dist < minDist) {
                minDist = dist;
                nearest = npc;
            }
    ...
    private void emitGraphicsObjects() {
        ...
        for (GraphicsObject go : gfxObjects) {
            ...
            if (!isWithinRadius(playerWp, wp, radius))
                continue;
    ...
    public void onGameObjectSpawned(GameObjectSpawned e) {
        if (!recording || !config.logGameObjects() || isInSceneGracePeriod())
            return;
    ...
    public void onGameObjectDespawned(GameObjectDespawned e) {
        if (!recording || !config.logGameObjects())
            return;
        GameObject obj = e.getGameObject();
    ...
    private void emitSceneObject(String type, String category, int id,
        ...
        if (blockedObjectIds.contains(id))
            return;
    ...
    public void onItemSpawned(ItemSpawned e) {
        if (!recording || !config.logGroundItems())
            return;
        ...
        event.put("itemKey", "item:" + item.getId() + ":" + wp.getX() + ":" + wp.getY() + ":" + wp.getPlane());
    ...
    private void parseWatchedIds() {
        ...
        for (int id : watchedVarbitIds)
            prevVarbitValues.put(id, client.getVarbitValue(id));
        for (int id : watchedVarpIds)
            prevVarpValues.put(id, client.getVarpValue(id));
    }
    ...
    public void onVarbitChanged(VarbitChanged e) {
        if (!recording || !config.logAllVarbitChanges())
            return;
